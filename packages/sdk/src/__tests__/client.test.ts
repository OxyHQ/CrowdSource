...
describe('uploads', () => {
  it('presigns, sends the bytes, completes with the digest, and returns no URL', async () => {
    const bytes = new TextEncoder().encode('evidence bytes');
    const digest = 'sha256:96b5f5e1c0e2f7a4a3f4dc1f9f6bf3d6b1b0d0f3b6b0f9d5b9d0f5e1a2b3c4d5';

    const { fetch, calls } = stubTransport([
      json(200, {
        uploadId: 'upload_abc',
        url: 'https://evidence.example.com/presigned?signature=abc',
        method: 'PUT',
        headers: { 'x-amz-meta-scope': 'evidence' },
      }),
    ...
    const asset = await client(fetch).uploads.upload({ bytes, mimeType: 'image/jpeg' });
    ...
    // §12.10: nothing an application can hold on to points at the bytes.
    expect(JSON.stringify(asset)).not.toContain('http');

    expect(calls[1]?.url).toBe('https://evidence.example.com/presigned?signature=abc');
    expect(calls[1]?.method).toBe('PUT');
    expect(calls[1]?.headers.get('x-amz-meta-scope')).toBe('evidence');
    expect(calls[2]?.url).toBe('https://api.crowdsource.test/v1/uploads/upload_abc/complete');
  });

  it('keys the upload by content, so the same bytes are one upload', async () => {
    const bytes = new TextEncoder().encode('evidence bytes');
    const { fetch, calls } = stubTransport([
      json(200, { uploadId: 'upload_abc', url: 'https://evidence.example.com/p' }),
      new Response(null, { status: 200 }),
      json(200, {
        uploadId: 'upload_abc',
    ...
    await client(fetch).uploads.upload({ bytes, mimeType: 'image/jpeg' });

    expect(calls[0]?.headers.get('idempotency-key')).toMatch(/^upload\.[0-9a-f]{64}$/);
  });
});
