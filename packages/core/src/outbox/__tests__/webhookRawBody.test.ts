 ...
 *
 * Deliveries are signed by `@crowdsource.you/core/testing`, which signs exactly the
 * way the service does, and travel over a real socket. Nothing at the boundary
 * is faked.
 */

import { createCrowdSourceSandbox, WebhookSimulator } from '../../testing/index.js';
...
describe.each(BACKENDS)('$name', (backend) => {
  ...
  async function decidedEvent(): Promise<unknown> {
    ...
    const { CrowdSource } = await import('../../index.js');
  ...
  describe('the receiver reads raw bytes', () => {
    it('reaches the moderation router with req.body still undefined', async () => {
      harness = await backend.createHarness({ webhookSecret: WEBHOOK_SECRET });
