/**
 * Recovery from a stale lazy-route chunk failing to load.
 *
 * ONE file, not a `.web`/`.native` pair: this app is a web export and there is no
 * native bundle for the split to serve. It reaches `window` directly for the same
 * reason.
 *
 ...
 * failed"), so navigating to a not-yet-loaded route — the case explorer, the
 * Trust & Safety table — dies with no recovery.
 *
...
const RELOAD_STAMP_KEY = 'crowdsource-console:chunkReloadAt';
