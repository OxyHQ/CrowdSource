 ...
 *
 * This guard exists because the failure it catches has already happened once in
 * this repository: a bypass added for local verification (`if (false && ...)`)
 * was swept into a commit by a concurrent `git add -A`, and for a while the
 * pushed branch let every reviewer surface render with no session at all.
 *
 * Nothing else would have caught it. The app compiles, lints, renders and passes
 * every other test with the redirect disabled — that is exactly what makes it
 * dangerous. So the condition itself is asserted here, as text, and the
 * short-circuit forms that neuter it are named.
 *
...
/** Whole-line `//` comments and block comments removed — see auth-invariants. */
...
describe('the auth gate', () => {
  it('read the root layout', () => {
    // Vacuity floor: a path that silently read an empty file would make every
  ...
  it('waits for cold boot before routing on the session', () => {
    // `isAuthenticated: false` is UNDETERMINED until this resolves; routing on
    // it early bounces a returning reviewer to sign-in on every reload.
