...
function readInitialJavascriptPaths() {
  ...
  for (const match of html.matchAll(sourcePattern)) {
    const pathname = new URL(match[1], "https://crowdsource.invalid").pathname;
...
function sourceGroup(source) {
  ...
  if (source.includes("/packages/contracts/")) return "@crowdsource.you/contracts";
  return "other";
}
...
function printReport(report) {
  console.log("CrowdSource reviewer bundle report\n");
