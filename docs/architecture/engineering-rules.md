...
**Rebuild `contracts` before believing a red typecheck.** `@crowdsource.you/contracts` is consumed through its BUILT `dist` — the backend and every entry point of `@crowdsource.you/core` import the published shape, never `src`. After any rebase or checkout that pulls in a contracts change, every other package still compiles against the previous build and reports the newly-landed symbols as missing (`TS2305`) in files nobody touched. It reads exactly like someone else's broken commit. `cd packages/contracts && bun run build` first; only an error that survives that is real.
...
- **A default must be a pinned version, never "whatever is current".** `DEFAULT_POLICY` in `packages/core/src/defaults.ts` names an immutable published version and MUST equal `BASELINE_POLICY_SET_ID`/`BASELINE_POLICY_VERSION` in the backend's `policyBaseline.ts`; `core/src/__tests__/defaults.test.ts` reads that file and asserts it. A resolved-at-ingress "latest" would move the policy under an application that changed nothing and would split §7.3's dedup key, giving one post two cases.
...
- **Port**: `3000` | **Domain**: `api.crowdsource.oxy.so` | **Reviewer**: Cloudflare **Worker** `crowdsource-frontend` at `crowdsource.oxy.so` (`packages/reviewer/wrangler.toml`) | **Console**: Worker `crowdsource-console` at `console.crowdsource.oxy.so` (`packages/console/wrangler.toml`), whose deploy job is gated on the repository variable `CROWDSOURCE_CONSOLE_WORKER == 'ready'` because it creates a Worker and claims a hostname in the zone that carries every live Oxy backend. Neither is a Pages project: a Pages project always serves `<project>.pages.dev` with no way to switch it off, and `workers_dev = false` is what leaves the real hostname as the only way in. Cloudflare writes the DNS record for a custom domain itself, and refuses a hostname that already has externally managed records (`code: 100117`) — so the release writes no DNS, and a hand-made record for either hostname blocks the deploy.
