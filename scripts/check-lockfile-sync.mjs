...
// existed only as advice, and skipping it contributed to publishing releases from
// states that were never committed, burning version numbers npm will never hand
// out again.
//
// WHY NOT `--frozen-lockfile`
//
// It is the obvious gate and it fails OPEN. Measured on bun 1.3.14 here, it exits
// 0 on every desync shape that has actually shipped: a workspace package's own
// `version` bumped without regenerating the lockfile, and a dependency range
// widened to something the lockfile does not record (^2.9.0 → >=2.9.0). It fails
// only when a range resolves to nothing at all — a resolution error, not a sync
// check. It would have caught none of the incidents while reading like
// enforcement. (The false positive on a synced lockfile that ~/Oxy/AGENTS.md
// records did not reproduce: five consecutive runs on unmodified HEAD passed.)
//
...
// this repository — bun leaves the stale value in bun.lock and reports "no
// changes", so `git diff --quiet` says clean and an install-only gate exits 0.
// Verified independent of the cache (warm, cold, and with node_modules removed)
// and independent of how dependents reference the package (`workspace:*` and a
// caret range on the published version behave identically).
//
...
// version bump and a release published from an uncommitted state.
//
// Churn is deliberately NOT accommodated. A reproduce-the-rewrite-twice scheme
// was built and then removed: the non-deterministic lockfile churn it absorbed
// never fired here (five runs) or in #734 (four runs, warm and cold cache), and
// machinery for a failure that does not occur reads as load-bearing to whoever
// touches it next. If churn ever does surface, the failure output below names
// every changed line, which is enough to recognise it.
